...
if(!"figS2b_DE_WT_10k.csv"%in%list.files()){
  ...
  write.csv(DE_WT,"figS2b_DE_WT_10k.csv")
}else{
  DE_WT <- read.csv("figS2b_DE_WT_10k.csv",r=1,h=1,stringsAsFactors=F)
}

if(!"figS2b_DE_KO_10k.csv"%in%list.files()){
  ...
  write.csv(DE_KO,"figS2b_DE_KO_10k.csv")
}else{
  DE_KO <- read.csv("figS2b_DE_KO_10k.csv",r=1,h=1,stringsAsFactors=F)
}
...
png("figS2b.png",height=2,width=2,units="in",res=1000,pointsize=6)
