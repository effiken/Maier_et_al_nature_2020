...
# determine samples of the cells that I have CITEseq on

sample_IDs <- unique(hum$ldm$dataset$cell_to_sample)
sample_IDs <- sample_IDs[!is.na(sample_annots[sample_IDs,]$HTO)]
sample_IDs <- sample_IDs[sample_annots[sample_IDs,]$notes!="ADT fail"]

# Load the CITEseq matrices

cells <- colnames(hum$ldm$dataset$umitab)[hum$ldm$dataset$cell_to_cluster%in%c("14","45","41")]

adt_list <- list()
for(samp_iter in sample_IDs){
  message("Loading sampleID ",samp_iter)
  samp_env <- new.env()
  fn <- paste("/users/andrew leader/google drive/merad/scRNAseq_analysis/compiled/clustering_data_lung3/lungdata_",
              samp_iter,".rd",sep="")
  load(envir=samp_env,fn)
  adt_list[[samp_iter]] <- samp_env$adttab
  rm(samp_env)
  colnames(adt_list[[samp_iter]]) <- paste(samp_iter,"_",colnames(adt_list[[samp_iter]]),sep="")
  adt_list[[samp_iter]] <- adt_list[[samp_iter]][,intersect(colnames(adt_list[[samp_iter]]),cells)]
}
...
names(clust2col) <- c("41","45","14")
clust2col[2:3] <- alpha(clust2col[2:3],.2)

png("fig4e.png",height=1.61,width=4.5,units="in",res=300,pointsize=6)
layout(matrix(1:3,nrow=1))
par(cex.lab=1.5,pin=c(1,1),xpd=NA,mgp=c(1.9,0.75,0))
plot(log2(1+adtmat["CD1c",]),log2(1+adtmat["CD141",]),
     pch=16,
     col=clust2col[hum$ldm$dataset$cell_to_cluster[colnames(adtmat)]],
...
s <- split(colnames(adtmat),list(
           factor(hum$ldm$dataset$cell_to_cluster[colnames(adtmat)],c("41","45","14")),
           factor(sample_annots[hum$ldm$dataset$cell_to_sample[colnames(adtmat)],]$tissue,c("Tumor","Normal")),
           factor(sample_annots[hum$ldm$dataset$cell_to_sample[colnames(adtmat)],]$patient_ID)))
...
x[grep("41.N",colnames(marker_means))] <- 2
x[grep("41.T",colnames(marker_means))] <- 1
x[grep("45.N",colnames(marker_means))] <- 4
x[grep("45.T",colnames(marker_means))] <- 3
x[grep("14.N",colnames(marker_means))] <- 6
x[grep("14.T",colnames(marker_means))] <- 5
...
s_group <- split(colnames(marker_means),factor(group,c("41.Tumor","41.Normal","45.Tumor","45.Normal","14.Tumor","14.Normal")))
...
png("fig_s4a.png",height=3.65,width=2.73,units="in",res=300,pointsize=6)
