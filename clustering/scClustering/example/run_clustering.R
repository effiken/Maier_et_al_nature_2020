...
setwd("~/GoogleDrive/work/shared/analysis/public_pbmc/")
...
#compile_data(input_path = "~/GoogleDrive/work/shared/data/pbnc_cite_seq_data/",libnames = "LW5/",output_path = "saved_clustering/",filtered_or_raw = "raw",cite_seq = T)

#libnames=setdiff(samples,c("122","123","180","181","186","187","197"))

data_l=read_multiple_mtx(c("~/GoogleDrive/work/shared/data/public/human/10x_data/"),c("Fresh_pbmc_68k","pbmc_33k_v1","pbmc_4k_v2","pbmc_6k_v1","pbmc_8k_v2"),cell_interval = c(1000,1e6),noise_interval = c(100,1e6))
save(data_l,file="tmp/tmp_data_l.rd")


model_name="public_pbmc_1"
pdf(paste(model_name,".pdf",sep=""))
cluster(data_l_path="tmp/tmp_data_l.rd",model_name = model_name,k=20,load_seed=F,running_mode="local",params=list(train_set_size  = 1000,test_set_size = 1000,insilico_gating = insilico_gating,genes_excluded = c(mts,malat,xist,metallothionein,variable_seg_light_chain,cell_cycle,hla,jchain),seeding_varmean_quantile=.92,min_umis_per_seeding_gene=50,genes_excluded_from_seeding = c(rps,ep),init_min_umis_quantile_range=c(0,.2),fixed_downsampling_min_umis=NA,reg=1e-5,n_init_seeds=1,max_n_cores=1,init_method="TGL_kmeans",km_reg=.1))
dev.off()
